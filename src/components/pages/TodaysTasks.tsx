...
export default function TodaysTasks() {
  ...
  return todaysTasks?.length === 0 ? (
    <div className="w-full text-center py-10">No Tasks for today | {today}</div>
  ) : (
    <ToggleList tasks={todaysTasks?.filter((t) => t?.delete === false)} onDelete={handleDelete} onToggle={handleChecked} />
  );
}
